...
function validateAiPromptVideoPrompt(value,replyPrompt){
  ...
  if(!/\d+\s*[-–]\s*\d+\s*s/i.test(videoPrompt))throw new Error('AI_PROMPT_VIDEO_PROMPT_TIMELINE_REQUIRED');
...
function pillarPrompt({pillar,research='',feedback='',performance='',mood='RANDOM',recentAiTips=[],recentAiPrompts=[]}){
  ...
  const rules={
    AI_PROMPT:`AI_PROMPT의 정체성은 "내 사진 한 장으로, 지금 당장 SNS에 올리고 싶어지는 강렬하고 세련된 사진을 만드는 프롬프트"다. AI 기술이나 트렌드 근거가 아니라 결과 이미지에 대한 욕구를 먼저 만든다. 20~30대 여성이 결과를 보자마자 저장하고 자기 사진으로 시도하고 싶을지, 장면 자체가 강하고 기억에 남는지를 내부적으로 판단하되 점수나 후보 JSON은 만들지 않는다.

강한 장소·상황, 예상 밖 스타일링, 실제 촬영 문법을 결합한다. 꽃시장과 블랙 드레스, 새벽 편의점과 럭셔리 스타일, 호텔 엘리베이터와 미니멀 드레스, 세탁소와 하이패션, 공항과 셀럽 파파라치처럼 직접 촬영하려면 장소·의상·촬영 준비가 필요한 결과를 우선한다. 장소만 바꾼 돌려막기, 평범한 AI 화보, 증명사진, 흔한 미소 셀카, 단순 색상·헤어 비교, 의미 없이 예쁜 판타지 화보는 피한다.

장면이 한 장에서 가장 강하면 1장으로 만들고, photo dump나 변화 과정처럼 여러 컷이 더 매력적일 때만 4컷·6컷·9컷을 사용한다. 포맷이 콘셉트를 결정하면 안 된다. 스마트폰 거울셀카, 직접 플래시, compact camera snapshot, paparazzi telephoto, handheld street photo, 28mm wide-angle, 50mm editorial portrait, 호텔 주변광, 편의점 형광등, 빗길 반사처럼 장면에 맞는 실제 촬영 문법을 1~2개만 고른다. 자연스러운 피부와 신체 비율, 현실적인 옷감·반사·조명, 약간 불완전한 프레이밍을 우선하고 plastic skin, fake HDR, 과도한 bokeh, 불가능한 신체와 반사를 금지한다.
...
video_prompt는 reply_prompt와 같은 인물·장소·의상·조명·세계관을 공유하는 Higgsfield 복붙용 영문 프롬프트다. 반드시 정확히 4줄만 사용하고 각 줄을 "Prompt: 내용", "Action: 내용", "Settings: 내용", "Negative: 내용" 형식과 순서로 작성한다. 라벨 다음 내용은 같은 줄에서 바로 시작하며 항목 내부 줄바꿈은 금지한다. Prompt에는 uploaded reference photo의 얼굴과 identity 유지, full face visible, 같은 장소·의상·스타일을 명시한다. Action에는 전체 5~10초 범위에서 "0-2s ... / 2-5s ..."처럼 구간별 실제 행동, 표정 변화, 마지막 장면과 카메라 움직임 또는 고정 상태를 설계한다. Settings에는 "Soul Cinematic, Motion 0.6, Style 0.85, Consistency 98 (face lock)"을 반드시 포함하고 장면에 맞는 조명을 덧붙인다. Negative에는 different face, blurry face, extra fingers, body distortion, abrupt cuts, teleportation, cartoon, nude 등 현실성과 얼굴 연속성을 깨는 요소를 간결하게 넣는다. 사진 프롬프트 뒤에 make a video를 붙이는 방식, 무의미한 슬로모션, 계속 카메라만 보기, 순간이동, 얼굴·옷·신체 변화, 물체 생성·소멸, 방향 급변, 과도한 모션블러를 금지한다. 최대 ${GENERATED_REPLY_PROMPT_MAX_CHARS}자 안에서 완결한다.
...
AI_PROMPT는 reply_prompt(사진용) 외에 video_prompt(영상용)도 반드시 함께 작성한다. 둘은 동일 콘셉트의 연장이어야 하며 동일 인물·장소·의상 continuity를 유지한다. video_prompt는 Higgsfield용 영문 형식으로 정확히 4줄만 사용하여 Prompt: 내용, Action: 내용, Settings: 내용, Negative: 내용을 이 순서로 작성하고 라벨과 내용을 같은 줄에 둔다. Action에는 초 단위 구간과 실제 행동·카메라 동작을 넣고, Settings에는 Soul Cinematic, Motion 0.6, Style 0.85, Consistency 98 (face lock)을 반드시 포함한다.
