...
reply_prompt는 사용자가 그대로 복사해 실행할 수 있는 완성형 전문 영문 실용 프롬프트다. 프롬프트 전체를 자연스럽고 명확한 영어로 작성하고 한국어 문장을 섞지 않는다. 반드시 "ROLE:", "INPUT MATERIALS:", "OBJECTIVE:", "ANALYSIS PROCESS:", "OUTPUT FORMAT:", "PRECAUTIONS:" 여섯 영문 항목을 포함하고 사용자가 자료를 붙이는 명확한 영문 자리표시자를 둔다. 실제 AI가 바로 수행할 수 있도록 구체적인 명령형 영어를 사용하되 불필요하게 장황하게 만들지 않는다. 근거 없는 단정 금지, 불확실성 표시, 민감정보 제거 등 해당 작업에 필요한 안전 조건도 영어로 명시한다.
...
function aiImageCtaInstruction(){
  return `FIXED CTA OVERLAY: A small text-only CTA will be composited directly over the finished photograph after image generation. Do not render, imitate, reserve or create any box, background plate, ribbon, banner, button, badge, footer strip, blank margin, solid-color area, gradient field or CTA color card for it. The photograph must remain one continuous full-bleed scene from the top edge through the bottom edge. Do not crop, shrink, push upward, reframe, clear, simplify or empty the lower part of the image for the CTA. People, scenery, props and natural photographic detail may continue normally behind the CTA. The CTA is allowed to have imperfect readability over the photograph; never alter the image composition to improve CTA readability. Keep generated hook/story text away from the extreme bottom edge so it does not collide with the later CTA, but reserve no dedicated footer area.`;
}
...
AI_TIP reply_prompt는 전문적인 영어로만 작성하고 한국어를 섞지 않는다. "ROLE:", "INPUT MATERIALS:", "OBJECTIVE:", "ANALYSIS PROCESS:", "OUTPUT FORMAT:", "PRECAUTIONS:"을 모두 포함한 완성형 복붙용 실용 프롬프트로 쓴다.
...
JSON만:
{"hook":"...","hook_candidates":["...","...","...","...","..."],"body":"...","reply_prompt":"AI_PROMPT is a detailed English image prompt; AI_TIP is a professional English practical prompt containing all six required sections","reason":"...","image_brief":"..."}`;
...
Place DISPLAY TEXT directly on authentic natural negative space already inside the photograph. TEXT BACKGROUND IS STRICTLY FORBIDDEN: no translucent panel, solid card, rectangle, box, ribbon, banner, strip, plate, gradient field, editorial caption block, header area, footer area or any separate background behind the text. Never crop, shorten, shrink, push, shift or reframe the photograph to manufacture a text area. The photograph must remain one continuous full-bleed image from top edge to bottom edge. Keep DISPLAY TEXT away from the extreme bottom edge where the later CTA will sit, but do not reserve, clear, simplify or manufacture any dedicated CTA zone. Reserve a generous no-text safety zone around every face, body and essential food/product/event subject. If readability is difficult, move the text to a different natural empty area or use only subtle text shadow/outline; never create a background for the text. Reposition the subject or camera only as part of the natural photographic composition, never to create a graphic header/footer region. Design the scene and its exact text together in the first generation pass. No Canvas, pasted headline or later story-text overlay is used; only the fixed text-only CTA is composited directly on top of the finished full-bleed photograph afterward, with no footer background or reserved margin.`;
}
