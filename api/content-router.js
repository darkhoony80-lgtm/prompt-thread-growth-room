...
reply_prompt에는 사용자가 그대로 복사할 수 있는 '영문 프롬프트만' 넣는다. 설명, 제목, 따옴표, Markdown, 'Prompt:' 같은 접두어, 한국어 번역은 넣지 않는다. 피사체/행동/환경/시간/카메라/렌즈/구도/조명/색감/질감/현실성/금지요소 중 필요한 요소를 구체화해 전문적인 영어로 작성한다. 흔한 cinematic, warm lighting 단순 나열은 금지. 길이 때문에 핵심 디테일을 억지로 삭제하지 말고 완성도 높은 복붙용 프롬프트를 만든다.`,
...
상단 약 25~30%는 제목을 '이미지 위에 직접' 얹을 수 있도록 시각적으로 비교적 단순한 배경으로 구성한다. 검은 띠, 빈 직사각형, 인위적인 패널, 프레임, 텍스트 박스처럼 보이는 공간을 절대 만들지 않는다. 배경은 장면의 자연스러운 하늘, 벽, 보케, 어두운 건물, 얕은 심도 등으로 이어져야 한다.
중요한 얼굴이나 핵심 사물은 제목과 겹치지 않게 중앙/하단에 배치하되, 사진 전체는 한 장의 자연스러운 풀블리드 이미지로 보여야 한다.
