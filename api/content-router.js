...
상단 약 28~32%는 정확한 한국어 후킹을 합성할 수 있는 깨끗한 negative space. 중요한 얼굴이나 사물을 그 영역에 두지 않는다.
핵심 피사체는 중앙/하단에 크게, 한눈에 소재가 읽히게 한다.
모바일 피드에서 0.5초 안에 시선을 멈추게 하는 광고 캠페인/매거진 커버급 구도, 강한 명암, 깊이감, 현실적인 재질과 피부 표현.
사람이 소재 이해와 감정 전달에 도움이 되면 매력적인 한국 성인 여성을 자연스럽게 활용해도 좋다. 단, 모든 이미지에 여성을 강제로 넣지 않는다.
여성을 사용할 때는 매번 같은 AI 모델처럼 보이지 않도록 얼굴형, 헤어, 의상, 표정, 앵글, 인물 비중을 소재에 맞게 다양화한다. 과도한 노출이나 성적 연출은 금지.
표정은 과장된 유튜브 썸네일식 충격 표정보다 호기심, 놀람, 만족, 집중 같은 자연스러운 감정을 우선한다.
뻔한 AI 로봇, 푸른 회로판, 홀로그램 뇌, 의미 없는 네온을 기본값으로 쓰지 않는다.
AI_PROMPT: 제공 프롬프트의 '완성 결과' 자체가 저장 욕구를 만들 정도로 고급스럽게. 인물이 핵심이면 세련된 여성 모델을 활용할 수 있다.
AI_TIP: 낯선 개념을 한 장면으로 직관화한다. 사람이 있으면 화면/오브젝트와 상호작용하게 하고 실제 읽을 수 있는 UI나 텍스트는 만들지 않는다.
