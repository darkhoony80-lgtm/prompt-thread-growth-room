...
function validateBodySearchHashtags(value){
  const body=String(value||'').trim();
  const lines=body.split(/\r?\n/);
  const last=String(lines[lines.length-1]||'').trim();
  const tags=last.match(/#[0-9A-Za-z가-힣_]+/g)||[];
  if(tags.length!==5||last.replace(/#[0-9A-Za-z가-힣_]+/g,'').trim()){
    throw new Error('BODY_SEARCH_HASHTAGS_INVALID');
  }
  return body;
}
...
function aiPromptRecentLines(input){
  return (Array.isArray(input)?input:[]).slice(0,20).map(v=>{
    ...
    const style=String(v?.style_context||'').replace(/\s+/g,' ').trim().slice(0,420);
    return [topic,hook,style].filter(Boolean).join(' | ');
  }).filter(Boolean);
}

function aiPromptMoodRule(mood='RANDOM'){
  const rules={
    RANDOM:`장면, 장소, 스타일링, 촬영 문법의 조합을 폭넓게 고르되 결과를 처음 보는 순간 저장하고 자기 사진으로 시도하고 싶을 만큼 강한 장면을 선택한다.`,
    HAPPY:`밝고 생기 있지만 평범한 미소 셀카가 아닌, 행동과 장소가 기억에 남는 장면을 선택한다.`,
    LOVE:`설렘, 애정, 우정의 감정이 행동과 상황으로 드러나는 세련된 장면을 선택한다.`,
    COMIC:`억지 밈이나 과장된 표정 대신 실제 촬영 같은 상황 안에서 재치가 생기는 장면을 선택한다.`,
    HORROR:`고어 없이 기묘함과 긴장감이 현실적인 장소, 조명, 행동에서 생기는 장면을 선택한다.`,
    FANTASY:`인물보다 효과가 앞서는 몽환 화보를 피하고, 현실적인 사진에 하나의 놀라운 시각 요소가 자연스럽게 결합된 장면을 선택한다.`
  };
...
const AI_PROMPT_PROMPT_STOPWORDS=new Set([
  'image','images','photo','video','prompt','videos','shot','create','creating','create','creatively','shot','scene','frames','framing','framed','camera','cam','lens',
  'lighting','light','focus','subject','person','woman','girl','voa','same','with','from','into','inside','outside','the','and','then','while','using','while',
  'for','that','this','these','those','there','here','into','their','there','between','around','because','without','through','after','before','during','first',
  'second','third','final','final','moment','sequence','result','results','full','body','look','looks','lighting'
]);

function extractPromptConceptTokens(value){
  const tokens=String(value||'').toLowerCase()
    .replace(/[^a-z0-9\s]/g,' ')
    .split(/\s+/)
    .map(v=>v.trim())
    .filter(v=>v.length>=4&&!AI_PROMPT_PROMPT_STOPWORDS.has(v));
  return new Set(tokens);
}

function hasPromptConceptOverlap(reference,next){
  const left=extractPromptConceptTokens(reference);
  const right=extractPromptConceptTokens(next);
  let overlaps=0;
  for(const token of right){
    if(left.has(token))overlaps++;
...
function validateAiPromptVideoPrompt(value,replyPrompt){
  ...
  if(!hasAction)throw new Error('AI_PROMPT_VIDEO_PROMPT_ACTION_REQUIRED');
  if(!hasCamera)throw new Error('AI_PROMPT_VIDEO_PROMPT_CAMERA_REQUIRED');
  if(!hasIdentity)throw new Error('AI_PROMPT_VIDEO_PROMPT_IDENTITY_REQUIRED');
  if(!hasSceneContinuity)throw new Error('AI_PROMPT_VIDEO_PROMPT_CONTEXT_MISMATCH');
}

function validateAiPromptCandidate(item){
  if(!item.body||!item.hook||!item.reply_prompt||!item.video_prompt||!item.image_brief){
    throw new Error('AI_PROMPT_REQUIRED_FIELD_MISSING');
  }
  if(item.reply_prompt.length>GENERATED_REPLY_PROMPT_MAX_CHARS)throw new Error('AI_PROMPT_MASTER_PROMPT_TOO_LONG');
  if(/[가-힣]/.test(item.reply_prompt))throw new Error('AI_PROMPT_REPLY_PROMPT_NOT_ENGLISH');
  if(!/PRIMARY IDENTITY REFERENCE/i.test(item.reply_prompt)||!/attached reference image/i.test(item.reply_prompt)){
    throw new Error('AI_PROMPT_IDENTITY_LOCK_MISSING');
  }
  if(/\b(?:young\s+(?:woman|man|girl|boy)|(?:east\s+asian|korean|asian|white|black)\s+(?:woman|man|girl|boy|person)|\d{1,2}(?:-year-old|\s+year\s+old)|female\s+model|male\s+model)\b/i.test(item.reply_prompt)){
    throw new Error('AI_PROMPT_IDENTITY_REDEFINITION');
  }
  validateAiPromptVideoPrompt(item.video_prompt,item.reply_prompt);
}
...
function pillarPrompt({pillar,research='',feedback='',performance='',mood='RANDOM',recentAiTips=[],recentAiPrompts=[]}){
  ...
  const rules={
    AI_PROMPT:`AI_PROMPT의 정체성은 "내 사진 한 장으로, 지금 당장 SNS에 올리고 싶어지는 현대적인 라이프스타일·패션 사진을 만드는 프롬프트"다. AI 기술을 과시하는 장면보다 한국의 20~40대 여성이 결과를 보자마자 저장하고 자기 사진으로 시도하고 싶은지를 먼저 판단한다. 점수나 후보 JSON은 만들지 않는다.

사진 한 장 안에 짧은 생활 서사와 감정이 보여야 한다. 누군가와 걷다가 돌아본 순간, 집에서 조용히 쉬는 순간, 계절과 날씨를 느끼는 순간, 일하거나 이동하는 순간, 선물이나 소품과 자연스럽게 상호작용하는 순간처럼 실제 하루에 있을 법한 행동을 중심에 둔다. 장소·의상·소품은 그 감정을 강화해야 하며, 서로 상관없는 강한 요소를 억지로 충돌시키지 않는다. 표정은 카메라를 계속 정면 응시하는 정형화된 화보보다 시선 이동, 작은 미소, 잠시 멈춤, 머리나 옷을 정리하는 자연스러운 반응을 우선한다.

촬영 문법은 장면마다 새로 선택한다. 스마트폰 전면 셀피, 스마트폰 후면 candid snapshot, 동행자가 찍어준 생활 사진, 35mm environmental portrait, 50mm natural portrait, 85mm beauty editorial, 낮은 시점, 위에서 내려다본 구도, 비나 유리 너머 촬영 중 가장 적합한 하나를 사용한다. 현대적인 디지털 색감, 실제 피부와 옷감·소품의 촉감, 해부학적으로 자연스러운 손과 체중 분배, 배경을 알아볼 수 있는 거리감을 중요하게 다룬다. 무조건적인 야간 직접 플래시, 파파라치, 꽃시장, 블랙 드레스, 과한 럭셔리 화보를 기본값으로 삼지 않는다. retro grading, plastic skin, fake HDR, 과도한 bokeh와 연출 티가 강한 포즈를 피한다.
...
video_prompt는 reply_prompt와 같은 인물·장소·의상·조명·세계관을 공유하는 20초 Higgsfield 복붙용 영문 프롬프트다. 반드시 정확히 4줄만 사용하고 각 줄을 "Prompt: 내용", "Action: 내용", "Settings: 내용", "Negative: 내용" 형식과 순서로 작성한다. 라벨 다음 내용은 같은 줄에서 바로 시작하며 항목 내부 줄바꿈은 금지한다. Prompt에는 uploaded reference photo의 얼굴과 identity 유지, full face visible, 같은 장소·의상·스타일을 명시한다. Action은 정확히 "0-5s ... / 5-10s ... / 10-15s ... / 15-20s ..." 네 구간으로 나누고, 시작 장면부터 실제 행동·표정 변화·카메라 움직임을 거쳐 20초 마지막 장면까지 자연스럽게 이어지게 한다. Settings에는 "Soul Cinematic, Motion 0.6, Style 0.85, Consistency 98 (face lock)"을 반드시 포함하고 장면에 맞는 조명을 덧붙인다. Negative에는 different face, blurry face, extra fingers, body distortion, abrupt cuts, teleportation, cartoon, nude 등 현실성과 얼굴 연속성을 깨는 요소를 간결하게 넣는다. 사진 프롬프트 뒤에 make a video를 붙이는 방식, 무의미한 슬로모션, 계속 카메라만 보기, 순간이동, 얼굴·옷·신체 변화, 물체 생성·소멸, 방향 급변, 과도한 모션블러를 금지한다. 전체는 최대 ${GENERATED_REPLY_PROMPT_MAX_CHARS}자 안에서 완결한다.
...
image_brief는 결과가 한눈에 강하게 보이도록 인물, 행동, 장소, 스타일링, 빛과 시각적 긴장감을 구체적으로 설명한다. source_notes는 필수가 아니며 필요 없으면 빈 배열로 둔다. 실제 참고한 공개 콘셉트가 있을 때만 간단히 기록하고, source_notes 때문에 생성이 실패해서는 안 된다.`,
  ...
  const aiPromptSchema=`{"candidate":{"category":"AI_PROMPT","topic":"...","topic_tag":"...","topic_tag_candidates":["...","...","..."],"hook":"...","hook_candidates":["...","...","...","...","..."],"body":"...","reply_prompt":"complete English PHOTO PROMPT","video_prompt":"complete English VIDEO PROMPT","reason":"...","image_brief":"...","source_notes":[],"score":{"stop":0,"save":0,"share":0,"comment":0,"follow":0,"novelty":0,"visual":0,"total":0}}}`;
...
AI_PROMPT는 reply_prompt(사진용) 외에 video_prompt(영상용)도 반드시 함께 작성한다. 둘은 동일 콘셉트의 연장이어야 하며 동일 인물·장소·의상 continuity를 유지한다. video_prompt는 20초 Higgsfield용 영문 형식으로 정확히 4줄만 사용하여 Prompt: 내용, Action: 내용, Settings: 내용, Negative: 내용을 이 순서로 작성하고 라벨과 내용을 같은 줄에 둔다. Action에는 0-5s / 5-10s / 10-15s / 15-20s 네 구간의 실제 행동·카메라 동작을 넣고, Settings에는 Soul Cinematic, Motion 0.6, Style 0.85, Consistency 98 (face lock)을 반드시 포함한다. 전체는 950자를 넘지 않는다.
기존:${JSON.stringify(x).slice(0,6000)}

JSON만:
{"hook":"...","hook_candidates":["...","...","...","...","..."],"body":"...","reply_prompt":"AI_PROMPT is a detailed English image prompt; AI_TIP은 한국어 실용 프롬프트이며 여섯 필수 항목을 모두 포함","video_prompt":"...","reason":"...","image_brief":"..."}`;
...
소재를 고를 때 먼저 "이 사실에서 어떤 이야기가 벌어지는가"를 찾는다. 인간의 욕망·실수·집착·배신·사기·생존·경쟁·기막힌 우연·황당한 선택·예상 밖의 결과·미스터리·반전·웃픈 실패처럼 서사를 움직이는 힘이 있어야 한다. 왕실·전쟁·첩보·재판·기업·발명·과학·스포츠·도시·재난·실종·사기·음식·동물·우주·한국 근현대사 등 실제 기록이 있는 모든 영역을 사용할 수 있다.
교과서 목차·보고서·학술 발표·박물관 전시 제목처럼 들리는 후보는 만들지 않는다. 하나의 강력한 테마로 여러 실제 사건을 엮는 옴니버스는 허용하되 제목과 one_line만으로 "그래서 무슨 일이 있었는데?"라는 궁금증이 생겨야 한다.
...
인물의 욕망·선택·갈등·실수·대가·반전·감정 변화를 우선하고 정보는 장면을 이해하는 데 필요한 순간에만 넣는다. 교과서식 정의, 연표 낭독, 기관 보고서 요약, "의의와 한계"식 결론을 피한다. 확인되지 않은 대화를 실제 기록처럼 만들지 않으며 재구성은 사실과 명확히 구분한다.

총 10~12개 scene으로 구성한다. scenes[].narration은 요약이 아니라 그대로 읽는 완전한 대본이며 전체 합계는 공백 포함 3,200~3,500자를 목표로 하고 반환 전에 최소 3,000자인지 확인한다. 의미 없는 반복으로 분량을 채우지 않는다. chapter.narration은 빈 문자열로 반환한다.

스토리를 먼저 설계한 뒤 각 scene을 나중에 무료·공개 소스로 구현할 방법을 설계한다. visual_description은 편집자가 찾아야 할 화면을 구체적으로 적는다. source_type은 photo|video|either 중 선택한다. source_queries는 무료 사진·영상 검색 API, 공개 아카이브, 정부·기관 자료 검색에 사용할 구체적인 영어 검색어 배열이다. 실제 인물·장소·사건·문서·신문·지도·공식 기록·아카이브 영상·일반 B-roll 중 장면에 적합한 자료를 찾을 수 있게 하고 특정 무료 API 하나에 종속되지 않게 한다. 확인되지 않은 사실을 시각자료가 증명하는 것처럼 꾸미지 않으며 이미지나 영상을 생성하지 않는다.
