...
const API='https://graph.threads.net/v1.0',WINDOW=2*24*60*60*1000,MAX_HISTORY=10,POST_PAGE_SIZE=50,MAX_POST_PAGES=10,MAX_POSTS_TO_CHECK=120;
...
async function history(token){const map=new Map(),ownReplyIds=new Set();let p=await graph(`/me/replies?fields=${encodeURIComponent('id,text,timestamp,replied_to')}&limit=50`,token),pages=0;while(p.ok&&pages<MAX_HISTORY){pages++;for(const x of (p.body?.data||[])){if(x?.id)ownReplyIds.add(String(x.id));const id=rel(x?.replied_to);if(id&&!map.has(id))map.set(id,{id:x?.id||null,text:x?.text||'',timestamp:x?.timestamp||null})}const n=p.body?.paging?.next;if(!n)break;p=await next(n,token)}return {ok:p.ok||pages>0,pages,map,ownReplyIds}}
...
async function discoverPosts(token,me,h,seedIds=[]){
 ...
 return {ids,pages,sourceCount,ok:p.ok||pages>0};
}
...
export default async function handler(req,res){
 ...
 const seedIds=[...new Set(String(req.query.post_ids||'').split(',').map(x=>x.trim()).filter(Boolean))].slice(0,10);
 ...
 if(!discovered.ids.length)return res.status(200).json({
  ok:true,window_days:2,count:0,unanswered_count:0,replied_count:0,review_count:0,
  reply_history_available:h.ok,reply_history_pages:h.pages,posts_checked:0,post_pages_scanned:discovered.pages,failures:[],items:[]
 });

 const mine=String(me.body.username||'').toLowerCase(),items=[],failures=[],seenReplies=new Set();
 ...
 return res.status(200).json({
  ok:true,window_days:2,count:items.length,
  ...
  posts_checked:discovered.ids.length,post_pages_scanned:discovered.pages,post_source_count:discovered.sourceCount,
  failures,items
 });
}
