...
async function createAttachment(token,replyTo,attachmentText,textValue){
  const params={
    media_type:'TEXT',
    reply_to_id:replyTo,
    text_attachment:{plaintext:attachmentText}
  };
  // Prefer an attachment-only reply. Add visible text only in the fallback attempt.
...
export default async function handler(req,res){
  ...
  // First try: no visible short comment, only the text attachment.
  let create=await createAttachment(s.accessToken,replyTo,attachmentText,'');
  let usedFallback=false;

  // Some API versions may still require a non-empty `text` for TEXT media.
  // If so, retry once with the smallest possible visible label.
  if(!create.ok){
  ...
  return res.status(200).json({
    ...
    fallback_label_used:usedFallback
  });
}
