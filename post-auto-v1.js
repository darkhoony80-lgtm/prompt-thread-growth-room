...
function topicTag(i){
 ...
 if(!x?.topic_tag_verified)return '';
 return String(document.getElementById(`v3topic-${i}`)?.value||x?.topic_tag||'').replace(/^#+/,'').trim().slice(0,80);
}
...
async function generatePillar(i,mood='RANDOM'){
 ...
 try{
  ...
  const recentFood=pillar==='FOOD_PICK'?read(FH,[]).slice(0,8):[];
   const r=await fetch('/api/content-router?action=generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pillar,feedback,performance,recentFood,mood:pillar==='AI_PROMPT'?mood:'RANDOM'})});
...
async function now(i){
 ...
 try{
  ...
  const r=await fetch('/api/threads/publish',{
   method:'POST',
   headers:{'Content-Type':'application/json'},
   body:JSON.stringify({text,image_url:url,topic_tag:topicTag(i),topic_tag_verified:candidates[i]?.topic_tag_verified===true})
  });
  const j=await r.json().catch(()=>({}));
