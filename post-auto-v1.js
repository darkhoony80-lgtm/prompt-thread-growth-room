...
async function generatePillar(i,mood='RANDOM'){
 ...
 finally{const nb=document.getElementById(`v3gen-${i}`);if(nb){nb.disabled=false;nb.textContent=pillar==='AI_PROMPT'?'🎲 랜덤':(candidates[i]?'↺ 리셋':'✨ 생성')}}
}
...
async function youtubeAdapter(snapshot){
 ...
 if(!tr.ok)throw new Error(apiError(tj,'YOUTUBE_TITLE_GENERATE_FAILED'));
 const title=String(tj.title||'').replace(/\s+/g,' ').trim().slice(0,100);
 if(!title)throw new Error('YouTube 제목 생성에 실패했습니다.');
...
function resetCard(i){
 ...
 if(!confirm('이 카드의 본문, 댓글/답장, 추천 Topic, 미디어 등 작업 내용을 모두 지울까요?'))return;
 const key=masterKey(i),records=masterRecords();delete records[key];write(CM,records);
 const ig=instagramRecords();delete ig[key];write(IGC,ig);
 if(instagramCarousel?.candidateId===key)instagramCarousel=null;
 candidates[i]=null;saveDrafts();render();
}
...
window.PostAuto={generate:generatePillar,reset:resetCard,image:i=>makeImage(i,false),reimage:i=>makeImage(i,true),keep,now,variant,no,drop,publishQueue,save:syncDraft,addMedia:addLocalMedia,removeMedia:removeMasterMedia,moveMedia:moveMasterMedia,instagram:openInstagramCarousel,instagramSelect:selectInstagramSlide,instagramCaption:setInstagramCaption,instagramReplyPrompt:setInstagramReplyPrompt,instagramClose:closeInstagramCarousel,instagramRegenerate:regenerateInstagramSlide,instagramRegenerateAll:regenerateInstagramCarousel,instagramPublish:publishInstagramCarousel,instagramMasterPublish:publishInstagramMaster,facebookPublish:publishFacebookMaster,youtubePublish:publishYoutubeMaster,instagramPromptStoreRetry:retryInstagramPromptStore};
