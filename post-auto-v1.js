...
const Q='pt_queue_v3',F='pt_feedback_v3',P='pt_published_v3',D='pt_drafts_v6',FH='pt_food_history_v1',IGC='pt_instagram_carousels_v1',CM='pt_content_masters_v1',AIC='pt_ai_prompt_custom_v1';
...
aiPromptCustomRequest=String(read(AIC,''));
...
async function generatePillar(i,mood='RANDOM',useCustom=false){
 const pillar=PILLARS[i],customAiPrompt=useCustom?aiPromptCustomRequest.trim():'',buttonId=useCustom?`v3customgen-${i}`:`v3gen-${i}`,b=document.getElementById(buttonId),s=document.getElementById('v3status');
 if(pillar==='AI_TIP'){
 ...
 if(useCustom&&!customAiPrompt)return alert('원하는 소재를 자연어로 입력해 주세요.');
 if(!b)return;b.disabled=true;const old=b.textContent;b.textContent='생성 중…';
 s.textContent=customAiPrompt?'입력한 소재로 사진·영상 프롬프트를 만드는 중입니다.':pillar==='HOT_ISSUE'?'오늘 뉴스를 검색하고 핫이슈를 고르는 중입니다.':pillar==='FOOD_PICK'?'전국 실제 맛집을 검색하고 오늘의 메뉴를 고르는 중입니다.':'이 섹션만 집중해서 콘텐츠를 만드는 중입니다.';
 try{
  ...
  const recentPrompts=pillar==='AI_PROMPT'?recentAiPrompts():[];
   const r=await fetch('/api/content-router?action=generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pillar,feedback,performance,recentFood,recentHotIssues:recentHot,recentAiTips:recentAi,recentAiPrompts:recentPrompts,mood:pillar==='AI_PROMPT'?mood:'RANDOM',customAiPrompt:pillar==='AI_PROMPT'?customAiPrompt:''})});
...
function aiPromptRequestHtml(i){return `<div class="cm-custom-request"><label for="v3custom-${i}"><b>원하는 소재 직접 지정</b></label><textarea id="v3custom-${i}" maxlength="500" oninput="PostAuto.customPrompt(this.value)" placeholder="예: 여권사진 4컷 / 사막에서 선인장에 기대어 있는 모습">${esc(aiPromptCustomRequest)}</textarea><button class="btn p" id="v3customgen-${i}" type="button" onclick="PostAuto.generateCustom(${i})">입력 소재로 생성</button><p class="mut">입력하지 않고 랜덤을 누르면 기존 방식으로 생성됩니다.</p></div>`}
function emptyCard(pillar,i){
 ...
 return `<article class="card" id="v3card-${i}"><div class="post-meta"><span class="badge">${esc(CATS[pillar])}</span></div><h3 style="margin:10px 0 6px">${esc(CATS[pillar])}</h3><p class="mut">${esc(desc)}</p>${localArchiveHtml(pillar,i)}${pillar==='AI_PROMPT'?`${aiPromptRequestHtml(i)}<div style="display:flex;gap:6px;flex-wrap:wrap">
...
function render(){
 ...
 if(!document.getElementById('v3mergeaudiocss'))document.head.insertAdjacentHTML('beforeend',`<style id="v3mergeaudiocss">.cm-bgm{margin-top:12px;padding:10px;border:1px solid var(--l);border-radius:10px}.cm-bgm-preview{display:block;width:100%;margin-top:8px}.cm-volume{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:5px;margin-top:10px}.cm-volume input{grid-column:1/-1;width:100%}</style>`);
...
async function saveContentArchive(i){
 ...
 try{
  ...
  let mergeBgm=null;
  if(draft?.bgmFile){const file=draft.bgmFile,extension=audioFileExtension(file),fileName=`merge-bgm.${extension}`;setContentArchiveStatus(i,'BGM 저장 중…');await writeArchiveFile(created.handle,fileName,file);mergeBgm={file_name:fileName,original_name:file.name,mime_type:file.type,size:file.size,original_volume:draft.originalVolume,bgm_volume:draft.bgmVolume}}
  const manifest={version:1,saved_at:new Date().toISOString(),content_type:PILLARS[i],hook:String(candidates[i]?.hook||''),title:String(candidates[i]?.topic||master.title||''),body:String(master.master_body||''),reply_prompt:String(master.reply_prompt||''),video_prompt:String(master.video_prompt||''),candidate:draftSafe(candidates[i]),master:{...master,media:archivedMedia},merge_videos:mergeVideos,merge_bgm:mergeBgm};
...
async function loadContentArchive(i){
 ...
 try{
  ...
  if(mergeFiles.length){let bgmFile=null,bgmPreviewUrl='';const bgm=manifest.merge_bgm;if(bgm?.file_name){const raw=await (await directory.getFileHandle(bgm.file_name)).getFile();bgmFile=new File([raw],bgm.original_name||raw.name,{type:bgm.mime_type||raw.type,lastModified:raw.lastModified});validateBgmFile(bgmFile);bgmPreviewUrl=URL.createObjectURL(bgmFile)}videoMergeDrafts.set(id,{files:mergeFiles,previewUrls:mergeFiles.map(file=>URL.createObjectURL(file)),bgmFile,bgmPreviewUrl,originalVolume:clampAudioVolume(bgm?.original_volume,30),bgmVolume:clampAudioVolume(bgm?.bgm_volume,70),encoding:false,status:'저장된 병합 순서로 불러왔습니다. 영상을 확인한 뒤 인코딩하세요.'})}
...
function releaseVideoMergePreviews(draft){for(const url of draft?.previewUrls||[]){if(url)URL.revokeObjectURL(url)}if(draft?.bgmPreviewUrl)URL.revokeObjectURL(draft.bgmPreviewUrl)}
...
function addVideoMergeFiles(i,fileList){
 ...
 try{
  ...
  const previewUrls=[...(current?.previewUrls||[]),...incoming.map(file=>URL.createObjectURL(file))];
  videoMergeDrafts.set(masterKey(i),{...current,files,previewUrls,originalVolume:clampAudioVolume(current?.originalVolume,30),bgmVolume:clampAudioVolume(current?.bgmVolume,70),encoding:false,status:'영상을 재생해 확인하고 순서를 정한 뒤 인코딩을 시작하세요.'});renderVideoMergeDraft(i);
...
function moveVideoMergeFile(i,index,direction){
 ...
 [draft.files[index],draft.files[to]]=[draft.files[to],draft.files[index]];[draft.previewUrls[index],draft.previewUrls[to]]=[draft.previewUrls[to],draft.previewUrls[index]];draft.status='변경된 순서로 인코딩할 준비가 됐습니다.';renderVideoMergeDraft(i);
}
function removeVideoMergeFile(i,index){
 const draft=videoMergeDraft(i);if(!draft||draft.encoding)return;if(draft.previewUrls?.[index])URL.revokeObjectURL(draft.previewUrls[index]);draft.files.splice(index,1);draft.previewUrls.splice(index,1);
 if(!draft.files.length){if(draft.bgmPreviewUrl)URL.revokeObjectURL(draft.bgmPreviewUrl);videoMergeDrafts.delete(masterKey(i))}else draft.status=draft.files.length<2?'영상을 하나 이상 더 추가하세요.':'순서를 확인한 뒤 인코딩을 시작하세요.';
 renderVideoMergeDraft(i);
}
function cancelVideoMerge(i){const draft=videoMergeDraft(i);if(!draft||draft.encoding)return;releaseVideoMergePreviews(draft);videoMergeDrafts.delete(masterKey(i));renderVideoMergeDraft(i);setVideoMergeStatus(i,'영상 병합 대기를 취소했습니다.')}
function audioFileExtension(file){return String(file?.name||'').split('.').pop().toLowerCase()}
function validateBgmFile(file){const mime=String(file?.type||'').toLowerCase(),extension=audioFileExtension(file);if(!['mp3','m4a','wav','aac','ogg'].includes(extension)||!(!mime||['audio/mpeg','audio/mp3','audio/mp4','audio/x-m4a','audio/wav','audio/x-wav','audio/aac','audio/ogg','application/ogg'].includes(mime)))throw new Error('BGM은 MP3, M4A, WAV, AAC, OGG 파일만 사용할 수 있습니다.');if(Number(file?.size||0)>100*1024*1024)throw new Error('BGM 파일은 100MB 이하여야 합니다.');return file}
...
async function mergeVideoFiles(files,status,{bgmFile=null,originalVolume=30,bgmVolume=70}={}){
 ...
 if(bgmFile)validateBgmFile(bgmFile);
 ...
 const normalizedNames=files.map((_,index)=>`merge-normalized-${index}.mp4`),bgmInput=bgmFile?`merge-bgm.${audioFileExtension(bgmFile)}`:'',concatOutput=bgmFile?'merge-concat.mp4':'merge-output.mp4',created=[...inputNames,...normalizedNames,'merge-list.txt',concatOutput,'merge-output.mp4',bgmInput].filter((value,index,array)=>value&&array.indexOf(value)===index);
 ...
 try{
  ...
  if(bgmFile)await ffmpeg.writeFile(bgmInput,await fetchFile(bgmFile));
...
function validateLocalMedia(file){
 ...
 if(type==='image/png'||type==='image/webp'){if(file.size>20*1024*1024)throw new Error(`${file.name}: PNG/WEBP 이미지는 20MB 이하여야 합니다.`);return 'image'}
 throw new Error(`${file?.name||'파일'}: JPEG/PNG/WEBP 사진 또는 MP4/MOV 영상만 추가할 수 있습니다.`);
}
async function imageToJpeg(file){
 const mime=String(file?.type||'').toLowerCase();
 if(!/^image\/(?:png|webp)$/.test(mime))return file;
 if(file.size>20*1024*1024)throw new Error(`${file.name}: ${mime.toUpperCase()} 이미지는 20MB 이하여야 합니다.`);
 const url=URL.createObjectURL(file);
  try{
   const image=await new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>reject(new Error(`${file.name}: 이미지 변환에 실패했습니다.`));img.src=url});
...
async function publishFirstReply(parentId,text,mode='text'){
 ...
 const r=await fetch('/api/threads/reply',{
  ...
  body:JSON.stringify(isAttachment
   ?{reply_to_id:String(parentId),attachment_text:text.trim()}
   :{reply_to_id:String(parentId),text:text.trim()})
 });
 const j=await r.json().catch(()=>({}));
 if(!r.ok)throw new Error(apiError(j,isAttachment?'FIRST_ATTACHMENT_REPLY_FAILED':'FIRST_REPLY_FAILED'));
 return j;
}
...
async function now(i){
 ...
 try{
  ...
  if(['AI_PROMPT','AI_TIP'].includes(PILLARS[i])){
   try{
    const isPrompt=PILLARS[i]==='AI_PROMPT';
    await publishFirstReply(j.id,payload.reply_prompt,isPrompt?'attachment':'text');
    replyMessage=isPrompt
...
async function youtubeAdapter(snapshot){
 ...
 const tr=await fetch('/api/content-router?action=youtube_title',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({body:description})});
...
function resetCard(i){
 ...
 const key=masterKey(i),records=masterRecords(),master=ensureMaster(i),mergeDraft=videoMergeDraft(i);
 if(mergeDraft){releaseVideoMergePreviews(mergeDraft);videoMergeDrafts.delete(key)}
 ...
 saveDrafts();render();
 const status=document.getElementById('v3status');if(status)status.textContent=`${CATS[PILLARS[i]]} 작업 내용이 초기화되었습니다.`;
}
