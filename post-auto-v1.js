...
function aiPromptRequestHtml(i){return `<div class="cm-custom-request"><label for="v3custom-${i}"><b>원하는 소재 직접 지정</b></label><textarea id="v3custom-${i}" maxlength="500" oninput="PostAuto.customPrompt(this.value)" placeholder="예: 여권사진 4컷 / 사막에서 선인장에 기대어 있는 모습">${esc(aiPromptCustomRequest)}</textarea><div style="display:flex;gap:7px;flex-wrap:wrap"><button class="btn p" id="v3customgen-${i}" type="button" onclick="PostAuto.generateCustom(${i})">입력 소재로 생성</button><button class="btn" type="button" onclick="PostAuto.reset(${i})" style="background:#e53955;color:#fff;border-color:#ff6b81;font-weight:800;box-shadow:0 0 0 1px #e5395544">🗑 AI 프롬프트 전체 리셋</button></div><p class="mut">입력하지 않고 랜덤을 누르면 기존 방식으로 생성됩니다.</p></div>`}
