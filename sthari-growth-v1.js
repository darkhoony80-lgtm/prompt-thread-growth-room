...
async function cycle(){if(!running||busy)return;busy=true;try{if(todayStats().done>=DAILY_LIMIT){stop('오늘 한도 40건 완료');return}const qs=queries();if(!qs.length){stop('검색어를 1개 이상 등록해 주세요');return}const q=qs[queryIndex++%qs.length];sthariNow.textContent=`"${q}" 최근 글 검색 중…`;const j=await search(q);saveStats({found:j.count||0});const d=j.diagnostic||{};const fw=d.four_way||{};const diag=`TAG/RECENT ${fw.TAG_RECENT?.count??'?'} · TAG/TOP ${fw.TAG_TOP?.count??'?'} · KEYWORD/RECENT ${fw.KEYWORD_RECENT?.count??'?'} · KEYWORD/TOP ${fw.KEYWORD_TOP?.count??'?'} · 최종 ${d.final_count??j.count??0}`;const s=state();let p=null,skip=0;for(const i of(j.items||[])){if(!qualifies(i)||s.processed?.[i.id]||userCount(s,i.username)>=SAME_USER_DAILY){skip++;continue}p=i;break}if(skip)saveStats({skipped:skip});if(!p){sthariNow.textContent=`"${q}" · ${diag} · 새 대상 없음`;log({username:'diagnostic',query:q,text:`${diag}${d.first_raw?.topic_tag?` · Topic ${d.first_raw.topic_tag}`:''}${d.first_raw?.timestamp?` · ${d.first_raw.timestamp}`:''}`,status:'진단'});return}const text=replyFor(p);sthariNow.textContent=`${diag} · @${p.username} 답글 + 리포스트 중…`;await act(p,text);markDone(p);log({username:p.username,query:q,text,status:'💬 + 🔁 완료',thread_id:p.id});sthariNow.textContent=`@${p.username} 처리 완료`}catch(e){saveStats({failed:1});log({username:'system',query:'',text:e.message,status:'실패'});sthariNow.textContent='오류 · '+e.message}finally{busy=false;if(running)timer=setTimeout(cycle,INTERVAL_MS)}}
