...
(function(){
const KEY='pt_sthari_growth_v1',LOG='pt_sthari_growth_log_v1';
const QUERIES=['스하리','반하리','또하리','팔로워1000명','팔로워 1000명','맞팔'];
...
function qualifies(i){return /(스하리|반하리|또하리|팔로워\s*1000|맞팔|천팔)/.test(String(i.text||'').toLowerCase())}
async function search(q){const r=await fetch('/api/threads/growth-search?q='+encodeURIComponent(q),{cache:'no-store'}),j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j?.hint||j?.detail?.message||j?.error||'SEARCH_FAILED');return j}
async function act(i,t){const r=await fetch('/api/threads/growth-action',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({thread_id:i.id,text:t})}),j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j?.detail?.message||j?.error||'ACTION_FAILED');return j}
async function cycle(){if(!running||busy)return;busy=true;try{if(todayStats().done>=DAILY_LIMIT){stop('오늘 한도 40건 완료');return}const q=QUERIES[queryIndex++%QUERIES.length];sthariNow.textContent=`"${q}" 최근 글 검색 중…`;const j=await search(q);saveStats({found:j.count||0});const s=state();let p=null,skip=0;for(const i of(j.items||[])){if(!qualifies(i)||s.processed?.[i.id]||userCount(s,i.username)>=SAME_USER_DAILY){skip++;continue}p=i;break}if(skip)saveStats({skipped:skip});if(!p){sthariNow.textContent=`"${q}" · 새 대상 없음`;return}const text=replyFor(p);sthariNow.textContent=`@${p.username} · 답글 + 리포스트 중…`;await act(p,text);markDone(p);log({username:p.username,query:q,text,status:'💬 + 🔁 완료',thread_id:p.id});sthariNow.textContent=`@${p.username} 처리 완료`}catch(e){saveStats({failed:1});log({username:'system',query:'',text:e.message,status:'실패'});sthariNow.textContent='오류 · '+e.message}finally{busy=false;if(running)timer=setTimeout(cycle,INTERVAL_MS)}}
...
function init(){renderStats();renderLog();setState('정지됨',false);sthariStart.addEventListener('click',start);sthariStop.addEventListener('click',()=>stop());sthariClearLog.addEventListener('click',()=>{localStorage.removeItem(LOG);renderLog()});window.addEventListener('beforeunload',()=>stop('브라우저 종료'))}
